...
//]]>
	...
	public static function GetScriptBlock ($src, $async=false)
	{
		// blocking js loading
		if (!$async)
		...
		if (!is_array($tiers))
			$tiers = array($tiers);
		if (is_array($tiers) && (count($tiers) > 0) && !is_array($tiers[0]))
			$tiers = array($tiers);
		if (count($tiers) == 0)
			return "";
		...
		$js = "";
		$last_hash = null;
		// non-blocking sequential js loading
		foreach($tiers as $sources) {
			$sources = '["' . implode('", "', $sources) . '"]';
			$hash = md5($sources);
			$_js = 'window.Zizio.$script(' . $sources . ', "' . $hash . '");';
			if ($last_hash)
				$_js = 'window.Zizio.$script.ready("' . $last_hash . '", function() { ' . $_js . '});';
			$js .= $_js; 
			$last_hash = $hash;
		}
	...
	private static function BuildScriptUrl($relative_url, $args=array(), $html_encoded=true, $include_def_args=true)
	{
		if ($html_encoded)
			list($eq, $sep) = array ("=", "&amp;");
		else
	...
	public static function GetAdminRegisterTermsUrl($args=array(), $html_encoded=true)
	{
		return self::BuildScriptUrl("/powers/mage/admin/terms", $args, $html_encoded);
	}

	public static function GetAdminRegisterScriptUrl($args=array(), $html_encoded=true)
	{
		return self::BuildScriptUrl("/js/mage/admin_register.js", $args, $html_encoded);
	}

	public static function GetZUtilsScriptUrl($args=array(), $html_encoded=true)
	{
		$args['v'] = "2.0";
		return self::BuildScriptUrl("/js/zutils.js", $args, $html_encoded, false);
	}

	public static function GetProductPageScriptUrl($args=array(), $html_encoded=true)
	{
		$args['v'] = "2.0";
		return self::BuildScriptUrl("/res/powers/mage/product", $args, $html_encoded);
	}

	public static function GetSuccessPageScriptUrl($args=array(), $html_encoded=true)
	{
		$args['v'] = "2.0";
		return self::BuildScriptUrl("/res/powers/mage/success", $args, $html_encoded);
	}

	public static function GetRegisterUrl($args=array(), $html_encoded=false)
	{
		return self::BuildScriptUrl("/powers/mage/admin/register", $args, $html_encoded);
	}

	public static function GetSystemConfigScriptUrl($args=array(), $html_encoded=false)
	{
		$args['v'] = "1.4";
		return self::BuildScriptUrl("/js/powers/mage/admin/config.js", $args, $html_encoded);
	}

	public static function GetAdminIncentiveGridScriptUrl($args=array(), $html_encoded=true)
	{
		$args['v'] = "2.0";
		return self::BuildScriptUrl("/js/powers/mage/admin/incentive.js", $args, $html_encoded);
	}

	public static function GetAdminIncentiveEditScriptUrl($args=array(), $html_encoded=true)
	{
		$args['v'] = "2.0";
		return self::BuildScriptUrl("/js/powers/mage/admin/incentive.js", $args, $html_encoded);
	}
