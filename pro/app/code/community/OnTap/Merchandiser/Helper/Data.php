...
class OnTap_Merchandiser_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function clearCategoryCache($categoryId)
    {
        if (Mage::getConfig()->getModuleConfig('Enterprise_PageCache')) {
            $versionVars = Mage::getVersionInfo();
            if ($versionVars['minor'] < 13) {
    ...
    {
        $xmlActions = Mage::getStoreConfig('merchandiser/actions');
        $actions = array();
    ...
    /**
     * isHideNoStockProducts
     * 
     * @return bool
     */
    public function isHideNoStockProducts()
    {
        return Mage::getStoreConfig('merchandiser/options/hide_no_stock');
    }
        ...
        if (is_object($curCategory)) {
            if (strlen(trim($categoryValues))>=1 || $categoryValues) {
                ...
                foreach ($attributeConditions as $attribute) {
                    ...
                    if($attribute['attribute'] == "new_product") {
                    	$currentDate = Mage::getModel('core/date')->date('Y-m-d');
                    	$productCollection->addAttributeToSelect(array('news_from_date', 'news_to_date'));
                    	$productCollection->addAttributeToFilter('news_from_date', array('or'=> array(
			                0 => array('date' => true, 'to' => $currentDate),
                    ...
                    }elseif ($attribute['attribute'] == "category_id") {
                        $productCollection
    ...
    {
        return Mage::getStoreConfig('merchandiser/options/rebuild_on_productsave');
    }
    ...
    {
        return Mage::getStoreConfig('merchandiser/options/rebuild_on_categorysave');
    }
    ...
    {
        return Mage::getStoreConfig('merchandiser/options/rebuild_on_cron');
    }
    ...
    {
        return Mage::getStoreConfig('merchandiser/options/new_products_handler');
    }
}