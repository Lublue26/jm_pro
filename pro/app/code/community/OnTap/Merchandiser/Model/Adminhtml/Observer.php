...
class OnTap_Merchandiser_Model_Adminhtml_Observer
{
    ...
    public function categorySaveAfter($observer)
    {
        ...
        if(Mage::helper('merchandiser')->isHideProductPositionField()){
        	$savedPostions = $category->getData('products_position');
        	foreach ($productPositions as $pKey => $pVal){
        		if(isset($savedPostions[$pKey])){
        			$productPositions[$pKey] = $savedPostions[$pKey];
        		}
        	}
        }
        ...
        if (Mage::helper('merchandiser')->rebuildOnCategorySave()) {
            ...
            if (sizeof($productPositions) > 0) {
                ...
                foreach ($productPositions as $productId ) {
                	if(!in_array($productId, $ruledProductIds) && $insertValues['ruled_only'] == 1){
                		continue;
                	}
                    $allocatedProducts[] = $productId;
