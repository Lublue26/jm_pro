...
class IllApps_Shipsync_Model_Shipping_Package_Item
{
	...
	$i = 0;
		...
		// 2012/03/19 Kenneth Roy
	    //$product = Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku());
	    //$product = $item->getProduct();
		// 2012/03/21 Kenneth Roy
	...
	if (!isset($_items) || !is_array($_items)) { return false; }
        ...
        if ($toShip)
	{
	    foreach ($_items as $item) { $quantity_count[$item['product_id']][] = $item; }
	...
	else { $_items = $packageSort->sortByKey('weight', $_items); }
    ...
    public function getOrigin($id)
    {
        if ($id != 0)
        {
            ...
            $shipperStreetLines = array(Mage::getStoreConfig('shipping/altorigin/address1'));
            if (Mage::getStoreConfig('shipping/altorigin/address2') != ''){$shipperStreetLines[]= Mage::getStoreConfig('shipping/altorigin/address2');}
            if (Mage::getStoreConfig('shipping/altorigin/address3') != ''){$shipperStreetLines[]= Mage::getStoreConfig('shipping/altorigin/address3');}
