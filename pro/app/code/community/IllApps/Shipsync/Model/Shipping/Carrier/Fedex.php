...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex extends Mage_Usa_Model_Shipping_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
	...
	if (!$this->getConfigFlag('active')) { return false; }
        ...
        if (Mage::getStoreConfig('carriers/fedex/disable_rating')) { return false; }
	...
	else
	{
	    ...
	    $this->setFedexMeter(Mage::getStoreConfig('carriers/fedex/meter'));
	    $this->setWsdlPath(dirname(__FILE__) . '/Fedex/wsdl/' . $wsdlPath);
	}

	try
	{
	    $soapClient = new SoapClient($this->getWsdlPath());
	}
	catch (Exception $ex)
	{
	...
	switch (Mage::getStoreConfig('carriers/fedex/units_of_measurement'))
	{
	    case 'LB_IN' : return 'LB';
	    case 'KG_CM' : return 'KG';
	    case  'G_CM' : return  'G';
            //default      : return 'LB';
	}
    }
    ...
    public function getCode($type, $code = '', $underscore = false)
    {
        $codes_underscore = array(	    
            'method' => array(
                ...
                'FEDEX_EXPRESS_SAVER' => Mage::helper('usa')->__('FedEx'),
        ...
        $codes = array(	    
            'method' => array(
                ...
                'FEDEXGROUND' => Mage::helper('usa')->__('FedEx'),
	...
	{
	    foreach ($allowedMethods as $method) { $parsedAllowedMethods[] = $this->getUnderscoreCodeFromCode($method); }
    ...
    {
        if(!Mage::getStoreConfig('carriers/fedex/alternate_rating_methods')) { return $this->getParsedAllowedMethods(); }
