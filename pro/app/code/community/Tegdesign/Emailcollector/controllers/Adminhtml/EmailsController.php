...
class Tegdesign_Emailcollector_Adminhtml_EmailsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function deleteAction() {
		...
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Record deleted'));
				$this->_redirect('*/*/');
			} catch (Exception $e) {
    ...
    public function massDeleteAction() 
    {
        ...
        if(!is_array($ids)) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Nothing selected'));
