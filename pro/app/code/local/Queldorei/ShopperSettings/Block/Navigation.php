...
//        $this->_parent = $parent;
    ...
    public function drawOpenCategoryItem($category, $level = 0, array $levelClass = null)
    {
		...
		if( $category->getId()== 245){
	     $aaa = '<a href="' . $this->getCategoryUrl($category) . '" id="gif">' . $this->escapeHtml($category->getName()) . '</a>' . "\n"; 
		$bbb = '';
		$c = Mage::getModel('catalog/category')->getCategories(245);
				if(count($c) > 0) {
					$bbb .= '<div class="slidingDiv" style="display:none;"><ul>';
					foreach ($c as $ca) {
						$bbb .= '<li><a href="'.$this->getCategoryUrl($ca).'">'.$this->escapeHtml($ca->getName()).'</a></li>';
					}
					$bbb .= '</ul></div>';
				}
			$html[1] = $aaa.$bbb;
		...
		//echo  $category->getId()."-".$this->getCurrentCategoryPath();
		}
		/* Load category by id*/
           ...
           $children = $this->_getLeftCategoryCollection()
            ...
            if ($hasChildren) {
                ...
                foreach ($children as $i => $child) {
				    ...
				    if (substr($child->getUrlKey(), 0, 4) == 'gift')
                        $folder[] = $this->mergeFolder($child);
                    else {
                        $htmlChildren .= $this->drawOpenCategoryItem($child, $level + 1, $class);
                    }
                }
                if (!empty($htmlChildren)) {
                    ...
                    if (count($folder) > 0) {
                        $html[2] .=
                            '<li class=closed><span class="folder">Gifts Guide</span>';
                        $html[2] .= '<ul>' . implode('', $folder) . '</ul></li>';
                    }
                    $html[2] .= '</ul>';
                }
            }
    ...
    public function mergeFolder($category)
    {
        $html[0] = '<li class="file">';
        $html[1] = '<span><a href="' . $this->getCategoryUrl($category) . '">' . $this->escapeHtml($category->getName()) . '</a></span>' . "\n";
        $html[2] = '</li>';
        return implode('', $html);
    }
